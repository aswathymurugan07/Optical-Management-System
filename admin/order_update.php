...
mysqli_query($con,"UPDATE orders set orderid='" . $_POST['orderid'] . "', email='" . $_POST['email'] . "', landmark='" . $_POST['landmark'] . "',c_address='" . $_POST['c_address'] . "' WHERE orderid='" . $_POST['orderid'] . "'");
$message = "Record Modified Successfully";
}

$result = mysqli_query($con, "SELECT * FROM orders where orderid='" . $_GET['id'] . "'");
...
<input type="text" name="email" placeholder="Email"class="txtField form-control" value="<?php echo $row['email']; ?>"></div>
<div class="mb-3">
<input type="text" name="c_address" placeholder="product price"class="txtField form-control" value="<?php echo $row['c_address']; ?>"></div>
<div class="mb-3">

<input type="text" name="landmark"placeholder="landmark" class="txtField form-control" value="<?php echo $row['landmark']; ?>">
    </div>
...
<a href="order_show.php">back to disply page</a>
    </button>
</form>
</body>
</html>